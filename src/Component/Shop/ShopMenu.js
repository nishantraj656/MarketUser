    ...
    _inslized=async()=>{ 
       ...
       let shop = await AsyncStorage.getItem('ShopID')
        if(value !=null && shop != null){
            ...
            let sql ="select product_table.price as price ,product_list_table.pic_1 as pic,product_table.unit as unit,product_table.p_list_id,product_table.product_table_id as productID,product_table.shop_id AS ShopID,product_list_table.p_name AS pName,shop_info_table.name  as sName from product_table INNER JOIN product_list_table on product_table.p_list_id = product_list_table.p_list_id INNER JOIN shop_info_table ON shop_info_table.shop_info_id  = product_table.shop_id where product_list_table.sub_category_id = "+value+" AND product_table.shop_id= "+shop;
